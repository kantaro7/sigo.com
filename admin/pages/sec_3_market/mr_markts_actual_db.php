...
$_POST["texto_1"]=htmlentities($_POST['texto_1'], ENT_QUOTES, "UTF-8");
$_POST["texto_2"]=htmlentities($_POST['texto_2'], ENT_QUOTES, "UTF-8");
...
$ori_chk=$vlr_chkbx_si;
if($_POST["ori_chk"]!=$vlr_chkbx_si) $ori_chk=$vlr_chkbx_no;
$act_chk=$vlr_chkbx_si;
if($_POST["act_chk"]!=$vlr_chkbx_si) $act_chk=$vlr_chkbx_no;

vrfy_upld_fl("img_foto", "Imagen 1", $accn, 409600);
...
if($msg!=""){
	...
	switch ($accn) {
		case "i":
			...
			$sql="INSERT INTO ".$_SESSION["tabla"]." (nombre, texto_1, texto_2, orientacion, orden, name_foto, shown) VALUES ('".$_POST["nombre"]."', '".$_POST["texto_1"]."', '".$_POST["texto_2"]."', '$ori_chk', ".$_POST["orden"].", '".$_FILES['img_foto']['name']."', '$act_chk')";
			//echo("sql [".$sql."] <br>");
		...
		case "m":
			...
			$sql="UPDATE ".$_SESSION["tabla"]."  SET nombre='".$_POST["nombre"]."', texto_1='".$_POST["texto_1"]."', texto_2='".$_POST["texto_2"]."', orientacion='$ori_chk', orden=".$_POST["orden"].", shown='$act_chk'  $str_img  WHERE id_".$_SESSION["tabla"]."=$clave";
			//echo("sql [".$sql."] <br>");
			$stmt1=$db_pdo->prepare($sql);
			if($stmt1===false){
				trigger_error($db_pdo->error, E_USER_ERROR);
