...
$tmn_chk=$vlr_chkbx_si;
if($_POST["tmn_chk"]!=$vlr_chkbx_si) $tmn_chk=$vlr_chkbx_no;
...
if($msg!=""){
	...
	switch ($accn) {
		case "i":
			...
			$sql="INSERT INTO ".$_SESSION["tabla"]." (descripcion, tamano, fila, orden, name_foto, shown) VALUES ('".$_POST["descripcion"]."', '$tmn_chk', ".$_POST["fila"].", ".$_POST["orden"].", '".$_FILES['img_foto']['name']."', '$act_chk')";
			//echo("sql [".$sql."] <br>");
		...
		case "m":
			...
			$sql="UPDATE ".$_SESSION["tabla"]."  SET descripcion='".$_POST["descripcion"]."', tamano='$tmn_chk', fila=".$_POST["fila"].", orden=".$_POST["orden"].", shown='$act_chk'  $str_img  WHERE id_".$_SESSION["tabla"]."=$clave";
			//echo("sql [".$sql."] <br>");
			$stmt1=$db_pdo->prepare($sql);
			if($stmt1===false){
				trigger_error($db_pdo->error, E_USER_ERROR);
