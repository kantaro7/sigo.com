...
if($msg!=""){
	...
	switch ($accn) {
		case "i":
			...
			$sql="INSERT INTO ".$_SESSION["tabla"]." (nombre, texto_1, texto_2, link_url, orden, name_foto_1, name_foto_2, shown) VALUES ('".$_POST["nombre"]."', '".$_POST["texto_1"]."', '".$_POST["texto_2"]."', '".$_POST["link_url"]."', ".$_POST["orden"].", '".$_FILES['img_foto_1']['name']."', '".$_FILES['img_foto_2']['name']."', '$act_chk')";
			//echo("sql [".$sql."] <br>");
			...
			if($status===false){
				...
				$rslt_3 = upld_image("img_foto_2", "Imagen 2", $id_lcl, "ft_2");
			}
			break;
		case "m":
			...
			$sql="UPDATE ".$_SESSION["tabla"]."  SET nombre='".$_POST["nombre"]."', texto_1='".$_POST["texto_1"]."', texto_2='".$_POST["texto_2"]."', link_url='".$_POST["link_url"]."', orden=".$_POST["orden"].", shown='$act_chk'  $str_img  WHERE id_".$_SESSION["tabla"]."=$clave";
			//echo("sql [".$sql."] <br>");
			$stmt1=$db_pdo->prepare($sql);
			if($stmt1===false){
				trigger_error($db_pdo->error, E_USER_ERROR);
