...
$_POST["descripcion"]=htmlentities($_POST['descripcion'], ENT_QUOTES, "UTF-8");
...
vrfy_upld_fl("img_foto", "Foto", $accn, 102400);
...
if($msg!=""){
	...
	switch ($accn) {
		case "i":
			...
			$sql="INSERT INTO ".$_SESSION["tabla"]." (descripcion, link_url, orden, name_foto, shown) VALUES ('".$_POST["descripcion"]."', '".$_POST["link_url"]."', ".$_POST["orden"].", '".$_FILES['img_foto']['name']."', '$act_chk')";
			//echo("sql [".$sql."] <br>");
		...
		case "m":
			...
			$sql="UPDATE ".$_SESSION["tabla"]."  SET descripcion='".$_POST["descripcion"]."', link_url='".$_POST["link_url"]."', orden=".$_POST["orden"].", shown='$act_chk'  $str_img  WHERE id_".$_SESSION["tabla"]."=$clave";
			//echo("sql [".$sql."] <br>");
			$stmt1=$db_pdo->prepare($sql);
			if($stmt1===false){
				trigger_error($db_pdo->error, E_USER_ERROR);
