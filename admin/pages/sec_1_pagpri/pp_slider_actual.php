...
<div id="actual_contenedor">
  ...
  <div>
    <form id="frm_actual" name="frm_actual" method="post" action="<?php echo("admin/pages/".$_SESSION["carpeta"]."/".$_SESSION["rprog"]."_actual_db.php"); ?>" onsubmit="return frm_vldt_<?php echo($_SESSION["rprog"]."('$accn');"); ?>" enctype="multipart/form-data" accept-charset="UTF-8">
        ...
        <div id="elmnt_case">
        	<input type="text" name="descripcion" id="descripcion" class="elmnt_elmnt" size="70" maxlength="150" placeholder="<?php echo(html_encode("Indique la descripción")); ?>" value="<?php echo(html_encode($reg_data["descripcion"])); ?>" tabindex="01" onKeyPress="if(event.keyCode==13){ document.getElementById('orden').focus(); } return frmt_alpha(event);" >
        </div>
			</div>
			<div id="elmnt_block">
        ...
        <div id="elmnt_case_sl">
          <div class="elmnt_foto"><input type="file" name="img_foto" id="img_foto" class="elmnt_file" size="2" maxlength="150" style="float: left;" ></div>
      		<div class="msg_image"><?php echo(html_encode("(Dimensiones: 1400 x 600 pixeles. Peso no mayor de 500 Kb. Tipo JPG o PNG.)")); ?></div>
        </div>
			</div>
