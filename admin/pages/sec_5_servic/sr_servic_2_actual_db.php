...
if($msg!=""){
	...
	switch ($accn) {
		case "i":
			...
			$sql="INSERT INTO ".$_SESSION["tabla"]." (programa, nombre, texto, orden, name_foto, shown) VALUES (".$_POST["programa"].", '".$_POST["nombre"]."', '".$_POST["texto"]."', ".$_POST["orden"].", '".$_FILES['img_foto']['name']."', '$act_chk')";
			//echo("sql [".$sql."] <br>");
		...
		case "m":
			...
			$sql="UPDATE ".$_SESSION["tabla"]."  SET programa=".$_POST["programa"].", nombre='".$_POST["nombre"]."', texto='".$_POST["texto"]."', orden=".$_POST["orden"].", shown='$act_chk'  $str_img  WHERE id_".$_SESSION["tabla"]."=$clave";
			//echo("sql [".$sql."] <br>");
			$stmt1=$db_pdo->prepare($sql);
			if($stmt1===false){
				trigger_error($db_pdo->error, E_USER_ERROR);
